...
HI_VOID* SAMPLE_VENC_1080P_CLASSIC_RTSP(HI_VOID* p)
{
    ...
    ******************************************/
	...
	HI_MPI_ISP_GetExposureAttr(0, &aeattr);
#if 1
	aeattr.stAuto.stAGainRange.u32Min = 0x400;
	aeattr.stAuto.stAGainRange.u32Max = 0x1600;
	aeattr.stAuto.stDGainRange.u32Min = 0x400;
	aeattr.stAuto.stDGainRange.u32Max = 0x1600;
	aeattr.stAuto.stSysGainRange.u32Min = 0x400;
	aeattr.stAuto.stSysGainRange.u32Max = 0x1800;
	aeattr.stAuto.u8Speed = 128;
#endif
	aeattr.stAuto.enAEMode = 0;
	aeattr.stAuto.u8Compensation  = 64;
	aeattr.stAuto.bManualExpValue = 0;
	aeattr.stAuto.u32ExpValue = 0xFF;
	aeattr.stAuto.u8Tolerance = 8;
	aeattr.stAuto.u16BlackSpeedBias = 0x40;
	HI_MPI_ISP_GetExposureAttr(0, &aeattr);
#endif
...
void *OsdDisplay(void *args)//每600ms更新osd显示的时间
{
	while(!g_s32Quit)
	{
		Sys_Osd_UpdateByCh(0);
		usleep(1000*600);
	}
	return NULL;
}
...
int main(int argc, char* argv[])
{
	...
	//SYS_Osd_Init();
	//SYS_Osd_Create(0);
	//pthread_create(&osd,NULL,OsdDisplay,NULL);
	while (!g_s32Quit)
	{
		nanosleep(&ts, NULL);
		EventLoop(s32MainFd);
