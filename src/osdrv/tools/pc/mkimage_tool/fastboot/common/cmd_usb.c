 ...
 */
#ifdef CONFIG_USB_STORAGE
int __LOADER_USB_Mandatory_Upgrd(void)
{
	printf("please define LOADER_USB_Mandatory_Upgrd()\n");
}
int LOADER_USB_Mandatory_Upgrd(void) __attribute__((weak, alias("__LOADER_USB_Mandatory_Upgrd")));
extern int LOADER_USB_Mandatory_Upgrd(void);
int do_usbburn(void)
{
...
int do_usb_dev(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
...
#ifdef CONFIG_USB_DEVICE
U_BOOT_CMD(
	usbdev,	2,	1,	do_usb_dev,
	"USB device",
	"start - start controller as an usb device\n"
);
#endif
